...
public class WriteTransaction extends XaTransaction implements NeoStoreTransaction
{
    ...
    protected void doPrepare() throws XAException
    {
        ...
        {
            if ( !record.inUse() && record.getFirstRel() !=
    ...
    {
        for ( NodeRecord record : nodeRecords.values() )
            lockReleaser.setFirstIds( record.getId(), record.getFirstRel(), record.getFirstProp() );
    }
    ...
    private void commitRecovered()
    {
        try
        {
            ...
            for ( Command.RelationshipCommand command : relCommands )
            {
                ...
                if ( command.getStartNode() != -1 || command.getEndNode() != -1 ) 
                {
            ...
            PrimitiveRecord primitive )
    {
        ...
        while ( nextProp != Record.NO_NEXT_PROPERTY.intValue() )
        {
            ...
            if ( !propRecord.isCreated() && propRecord.isChanged() )
            {
                ...
                cachePropertyRecord( propRecord );
            }
...
//>>>>>>> master
            ...
            PrimitiveRecord primitive, RecordAdder adder )
    {
        ...
        long firstProp = primitive.getFirstProp();
...
//>>>>>>> master
    ...
    {
        if ( node.getId() == rel.getStartNode() ) rel.setStartNodeNextRel( nextRel );
        if ( node.getId() == rel.getEndNode() ) rel.setEndNodeNextRel( nextRel );
    }
    ...
    private RelationshipGroupRecord createRelationshipGroupRecord( NodeRecord node, int type )
    {
        ...
        long firstGroupId = node.getFirstRel();
    ...
    private void connect( long nodeId, long firstRelId, RelationshipRecord rel )
    {
        ...
        if ( firstRelId != Record.NO_NEXT_RELATIONSHIP.intValue() )
        {
            ...
            if ( firstRel.getEndNode() == nodeId )
            {
                ...
                firstRel.setFirstInEndNodeChain( false );
                changed = true;
            }
            if ( !changed )
            {
...
//        name.getChars( 0, length, chars, 0 );
    ...
    private boolean assertPropertyChain( PrimitiveRecord primitive )
    {
        ...
        long nextIdToFetch = primitive.getFirstProp();
        ...
        if ( toCheck.isEmpty() )
        {
            assert primitive.getFirstProp() == Record.NO_NEXT_PROPERTY.intValue() : primitive;
            return true;
        }
    ...
    {
        NodeRecord node = getNodeRecord( id, true, false );
    ...
    private int relCount( long nodeId, long relId )
    {
        ...
        return relCount( nodeId, getRelationshipRecord( relId, true, false ) );
    }
    
    private int relCount( long nodeId, RelationshipRecord rel )
    {
        return (int) (nodeId == rel.getStartNode() ? rel.getStartNodePrevRel() : rel.getEndNodePrevRel());
    }
    
    private boolean relIsFirstInChain( long nodeId, RelationshipRecord rel )
    {
        return (nodeId == rel.getStartNode() && rel.isFirstInStartNodeChain()) ||
    ...
    {
        Map<Integer, RelationshipGroupRecord> groups = getRelationshipGroups( getNodeRecord( id, true, false ) );
